...
(define (helperForPower relation k t)
  (cond
    [(equal? k 0) '()]
    [(equal? k 1) t]
    [else (helperForPower relation (- k 1) (compose relation t))]))
  
(define (power k relation)
  (helperForPower relation k relation))

(define (transitive-closureHelper relation k t)
  (if (subset? (power k relation) t)
      t
     (transitive-closureHelper relation (+ k 1) (union (power k relation) t))))

(define (transitive-closure R)
  (transitive-closureHelper R 1 '()))                 
...
(define (equivalence? n R)
  (and (and (reflexive? n R) (symmetric? R)) (transitive? R)))
...
(define (involutive? R n)
  (and (and (injective? R) (symmetric? R)) (surjective? n R)))

(define (surjective? n R)
  (define domain (map car R))
  (= (length domain)
     (cardinality (id n))))
