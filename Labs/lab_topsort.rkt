...
(define (indegree E v)
  (cond
    [(null? E) 0]
    [(equal? (car (cdr (car E))) v) (+ 1 (indegree (cdr E) v))]
    [else (+ 0 (indegree (cdr E) v))]
   ))
...
;; 5 PTS
(define (find-top G)
  (find-topHelper (car G) G))

(define (find-topHelper G V)
  (cond
    [(null? G) (topsort-error)]
    [(equal? (indegree (get-edges V) (car G)) 0) (car G)]
    [else (find-topHelper (cdr G) V)]
   ))
