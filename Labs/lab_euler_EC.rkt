...
(define (get-vertices G)
  (usort (get-verticesHelper G))
  )

(define (get-verticesHelper G)
  (if (equal? G '()) '()
      (append (car G) (get-verticesHelper (cdr G)))
      )
  )
...
(define (degree G v)
  (cond
    [(equal? G '()) 0]
    [(equal? (car G) (list v v)) (+ 2 (degree (cdr G) v))]
    [(equal? (car (car G)) v) (+ 1 (degree (cdr G) v))]
    [(equal? (car (cdr (car G))) v) (+ 1 (degree (cdr G) v))]
    [(+ 0 (degree (cdr G) v))]
    )
)

(define (all-even? G)
  (all-evenHelper? (get-vertices G) G))

(define (all-evenHelper? G H)
  (cond
    [(equal? G '()) #t]
    [(equal? (modulo (degree H (car G)) 2) 0) (all-evenHelper? (cdr G) H)]
    [else #f]
    )
)
...
(define (adjacent G v)
  (cond
    [(equal? G '()) '()]
    [(equal? (car G) (list v v)) (usort (append (list v) (adjacent (cdr G) v)))]
    [(equal? (car (car G)) v) (usort(append (list (car (cdr (car G)))) (adjacent (cdr G) v)))]
    [(equal? (car (cdr (car G))) v) (usort(append (list (car (car G))) (adjacent (cdr G) v)))]
    [else (usort (append (adjacent (cdr G) v)))]
   )
  )
...
(define (connected? G)
  (cond
    [(equal? G '()) #t]
    [(equal? (usort (dfs G (car (car G)))) (get-vertices G)) #t]
    [else #f]
    )
  )
